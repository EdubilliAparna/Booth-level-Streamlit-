...
# LOAD DATA
# ----------------------------------------------------
uploaded_file = st.file_uploader("Upload Booth-Level Dataset (CSV)", type=["csv"])
...
if uploaded_file is not None:
    ...
    if vote_cols:
        for col in vote_cols:
            dist_df[f"{col}_share"] = dist_df[col] / dist_df["valid_votes"]
        ...
        dist_df["margin_share"] = dist_df["margin"] / dist_df["valid_votes"]
        ...
        st.dataframe(
            dist_df[["polling_station_name", "winner", "margin_share", "cluster"]]
        )

    else:
        st.warning("No *_votes columns found")
    ...
    try:
        ...
        sns.heatmap(clust_count, annot=True, fmt="d", cmap="viridis", ax=ax2)
        st.pyplot(fig2)
